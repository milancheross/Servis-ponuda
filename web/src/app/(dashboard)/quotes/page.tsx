...
const FILTERS = [{ value: '', label: 'Sve' }, { value: 'nacrt', label: 'Nacrt' }, { value: 'poslata', label: 'Poslata' }, { value: 'prihvacena', label: 'Prihvaćena' }, { value: 'odbijena', label: 'Odbijena' }]
...
export default function QuotesPage() {
  ...
  useEffect(() => {
    ...
    fetch(url, { credentials: 'include' }).then(r => r.json()).then(d => { setQuotes(d.quotes || []); setLoading(false) })
  }, [filter])
              ...
              <tbody className="divide-y divide-gray-50">
                {quotes.map((q: any) => (
                  <tr key={q.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 font-medium"><Link href={`/quotes/${q.id}`} className="hover:text-blue-600">{q.client?.name || '—'}</Link></td>
